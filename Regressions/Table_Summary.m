...
load dataset.mat
load delta_yield.mat ts_left ts_right
format bank
nbond = size(dataset,2)-3;

ISIN = dataset{1}(:,"ISIN");
ISIN = table2array(ISIN);
CUSIP = dataset{1}(:,"CUSIP");
Maturity = dataset{1}(:,"Scadenza");
Maturity = datestr(datenum(table2array(Maturity)));
Benchmark = dataset{1}(:,"DataBenchmark");
Benchmark = datestr(datenum(table2array(Benchmark)));
Coupon = dataset{1}(:,"Cedola");
...
openvar("summary")
writetable(summary,'Summary_Table.xlsx','WriteRowNames',true)