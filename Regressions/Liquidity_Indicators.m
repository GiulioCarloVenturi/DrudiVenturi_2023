...
load dataset.mat
    ...
    clearvars -except liquidity_indicators ts_indicators
    
    %% Standardize Indicators
    Time = ts_indicators.time;
    nind = size(ts_indicators,2);
    Var = ts_indicators.Variables;
    mu = mean(Var);
    sd = std(Var);
    Var_std = zeros(size(Var));
    names = {'Bid_Ask',...
        'Bid_Ask_m','Bid_Ask_ftm','Roll','Roll_m','Roll_ftm',...
        'Corwin_Schultz','Corwin_Schultz_m','Corwin_Schultz_ftm',...
        'Abdi_Ranaldo','Abdi_Ranaldo_m','Abdi_Ranaldo_ftm',...
        ...
        'Volatility_ftm','Volume','Volume_m','Volume_ftm'};
    for i = 1 :nind
    Var_std(:,i) = (Var(:,i)-mu(i))./sd(i);
    end
    Var_std = array2table(Var_std);
    ts_indicators_std = table2timetable(Var_std,"RowTimes",Time);
    ts_indicators_std.Properties.VariableNames = names;
    clearvars -except liquidity_indicators ts_indicators_std ts_indicators
    
    save liquidity_indicators.mat liquidity_indicators ts_indicators ts_indicators_std